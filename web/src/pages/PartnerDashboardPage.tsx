...
type Transaction = { reference: string; type: string; status: string; amount: number; description: string; created_at: string };
...
export default function PartnerDashboardPage() {
  ...
  async function loadEverything() {
    ...
    setPartner(me.data);
    const [walletRes, keysRes, webhookRes, txRes] = await Promise.all([
      ...
      portalFetch<{ data: Transaction[] }>('/recent-transactions?limit=10').catch(() => null)
    ]);
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold">Major Data Link</p>
      ...
      <div className="mx-auto max-w-6xl px-5 py-8">
        ...
        <section className="rounded-3xl bg-gradient-to-br from-brand-800 to-brand-600 p-6 text-white">
          ...
          <p className="text-sm text-blue-100">Partner wallet balance</p>
        </section>
          ...
          >
            ...
            <div>
              <p className="font-bold">
                {partner.status === 'pending_review' ? 'Ana bitar rajistarku' : 'An dakatar da wannan asusun'}
              </p>
              <p className="mt-1 text-sm leading-relaxed">
                {partner.status === 'pending_review'
                  ? "Za a fara duba kamfaninku kafin ku iya ƙirƙirar live API key ko fara sarrafa transactions. Za mu tuntube ku ta imel ('" +
                    partner.email +
                    "') idan an amince."
                  : 'Tuntuɓi Major Data Link don ƙarin bayani game da wannan asusun.'}
              </p>
            </div>
          </section>
        )}
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 bg-white">
          ...
          {transactions.length ? (
            <div className="divide-y divide-slate-100">
              {transactions.map((tx) => (
                <div key={tx.reference} className="flex items-center justify-between gap-3 px-5 py-3">
                  <div>
                    <p className="font-mono text-xs font-semibold">{tx.reference}</p>
                    <p className="mt-1 text-xs uppercase text-slate-500">{tx.type.replaceAll('_', ' ')}</p>
                  </div>
            ...
            <p className="px-5 py-10 text-center text-sm text-slate-500">Babu transaction tukuna.</p>
          )}
        </section>
      </div>
    </main>
    ...
    <main className="min-h-screen bg-slate-50 px-5 py-10 text-slate-900">
      <section className="mx-auto max-w-md rounded-3xl border border-slate-200 bg-white p-7 shadow-xl shadow-slate-200/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-[.18em] text-brand-700">Major Data Link</p>
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          {mode === 'register' && (
            <div>
              ...
              <input
                ...
                placeholder="e.g. Kindness Digital Branding"
          ...
          <div>
            ...
            <div className="mt-2 flex rounded-lg border border-slate-300 focus-within:border-brand-500">
              <input
                ...
                placeholder={mode === 'register' ? 'Aƙalla haruffa 8' : '••••••••'}
          ...
          >
            ...
            {mode === 'login' ? 'Log in' : 'Ƙirƙiri asusun kamfani'}
          </button>
        </form>

        {mode === 'register' && (
          <p className="mt-5 text-xs leading-relaxed text-slate-500">
            Bayan rajista, admin din Major Data Link zai fara duba kamfaninku kafin ku iya ƙirƙirar live API key ko
            sarrafa transactions - amma za ku iya shiga dashboard ɗin nan take don duba matsayin rajistarku.
          </p>
        )}
      </section>
    </main>
...
function WalletCard({
  ...
  async function createFundingAccount() {
    ...
    try {
      ...
      onNotice('An kirkiri permanent funding account.');
    } catch (err) {
      onError(err instanceof Error ? err.message : 'Could not create funding account');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function createExactTransfer(event: FormEvent) {
    ...
    try {
      ...
      onError(err instanceof Error ? err.message : 'Could not create Exact Transfer account');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5">
      <div className="flex items-center gap-2">
        ...
        <h2 className="font-bold">Partner funding account</h2>
      </div>

      {wallet?.virtual_account_number ? (
        <div className="mt-4 rounded-xl bg-blue-50 p-4">
          <p className="text-xs font-semibold text-slate-500">{wallet.virtual_account_bank ?? 'Bank transfer'} · Permanent account</p>
          <p className="mt-2 font-mono text-xl font-bold">{wallet.virtual_account_number}</p>
          <p className="mt-1 text-xs text-slate-600">Transfer to this account to fund your Partner wallet.</p>
          <button
        ...
        >
          <WalletIcon size={16} />
          {busy ? 'Please wait…' : 'Create funding account'}
        </button>
      )}

      <div className="mt-5 border-t border-slate-100 pt-4">
        <p className="text-xs font-semibold text-slate-600">Exact Transfer (fallback, one-off amount)</p>
        ...
        {exactTransfer && (
          <div className="mt-3 rounded-xl bg-amber-50 p-3 text-sm">
            <p className="font-semibold">
              ₦{exactTransfer.amount.toLocaleString()} → {exactTransfer.bank_name ?? 'Bank'}
            </p>
            <p className="mt-1 font-mono font-bold">{exactTransfer.account_number}</p>
            <p className="mt-1 text-xs text-slate-600">
              Ref: {exactTransfer.reference}
              {exactTransfer.expires_at ? ` · Expires ${new Date(exactTransfer.expires_at).toLocaleString()}` : ''}
            </p>
          </div>
        )}
      </div>
...
function ApiKeysCard({
  ...
  async function generate() {
    ...
    try {
      ...
      onError(err instanceof Error ? err.message : 'Could not generate API key');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function revoke(id: string) {
    ...
    try {
      ...
      const refreshed = await portalFetch<{ data: ApiKey[] }>('/api-keys');
      onKeys(refreshed.data);
      onNotice('Key revoked.');
    } catch (err) {
      onError(err instanceof Error ? err.message : 'Could not revoke key');
    } finally {
      setBusy(false);
    }
  }
    ...
    <article className="rounded-2xl border border-slate-200 bg-white p-5">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          title={active ? undefined : 'Za a fara amincewa da kamfaninku kafin ku iya ƙirƙirar live key'}
      ...
      {revealedKey && (
        <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4">
          <p className="text-xs font-bold text-amber-800">Ajiye wannan key ɗin yanzu - ba za a sake nuna shi ba:</p>
      ...
      <div className="mt-4 space-y-2">
        {keys.length === 0 && <p className="text-sm text-slate-500">Babu API key tukuna.</p>}
        {keys.map((k) => (
...
function WebhookCard({
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      onError(err instanceof Error ? err.message : 'Could not save webhook URL');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function sendTest() {
    ...
    try {
      ...
      onNotice('Test event queued - duba endpoint ɗinku a cikin dakiku kaɗan.');
    } catch (err) {
      onError(err instanceof Error ? err.message : 'Could not queue test event');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5">
      ...
      <p className="mt-1 text-xs text-slate-500">
        Za a tura muku <code>transaction.updated</code> event idan wani transaction da farko ya tsaya "pending" ya
        warware.
      </p>
      ...
      <form onSubmit={save} className="mt-4 flex gap-2">
        <input
          ...
          type="url"
          placeholder="https://yourapp.com/webhooks/mdl"
      ...
      {revealedSecret && (
        <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4">
          <p className="text-xs font-bold text-amber-800">Webhook secret (ajiye yanzu, ba za a sake nuna shi ba):</p>
          <div className="mt-2 flex items-center gap-2 rounded-lg bg-white px-3 py-2">
