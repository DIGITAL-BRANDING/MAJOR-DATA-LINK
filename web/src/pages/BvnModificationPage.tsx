 ...
 * BvnModificationPage.tsx - same /api/bvn-modification endpoints, restyled
 * onto MAJOR-DATA-LINK's own ink/parchment/gold theme (matching the other
 * ported pages) instead of infoverify's navy/gold palette and shared
 * verification/shared.tsx helpers (which don't exist here).
 */
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-5xl">
        <button
          onClick={() => {
            if (stage === 'form' || stage === 'select' || stage === 'verify') return setStage('decide');
            nav('/dashboard');
          }}
          className="font-body text-sm font-semibold text-gold-700"
        >
          ← {stage === 'decide' ? 'Dashboard' : 'Start over'}
        </button>
        ...
        {stage === 'decide' && (
          <section className="mt-6 rounded-2xl border border-parchment-line bg-parchment p-6">
            <h2 className="font-display text-lg font-bold text-ink">Do you already know exactly what needs fixing?</h2>
            <p className="mt-2 font-body text-sm text-ink-600">
        ...
        {stage === 'verify' && (
          <section className="mt-6 rounded-2xl border border-parchment-line bg-parchment p-6">
            <h2 className="font-display text-lg font-bold text-ink">Check BVN &amp; NIN together</h2>
            <p className="mt-2 font-body text-sm text-ink-600">
              ...
              <form onSubmit={prepareVerify} className="mt-5 grid gap-4 sm:grid-cols-2">
                <label className="block font-body text-sm font-medium text-ink-600">
                  ...
                  <input
                    ...
                    value={verifyBvn}
                    onChange={(e) => setVerifyBvn(e.target.value.replace(/\D/g, '').slice(0, 11))}
                  />
                </label>
                <label className="block font-body text-sm font-medium text-ink-600">
                  NIN Number
                  <input
                    ...
                    className="mt-1 w-full rounded-xl border border-parchment-line bg-cream p-3 text-ink outline-none focus:border-gold-500"
                    value={verifyNin}
                    onChange={(e) => setVerifyNin(e.target.value.replace(/\D/g, '').slice(0, 11))}
                  />
                </label>
                <div className="sm:col-span-2">
                  <button
                    disabled={verifyBusy}
                    className="flex w-full items-center justify-center gap-2 rounded-xl bg-gold-500 py-3 font-display font-semibold text-ink disabled:opacity-60 sm:w-auto sm:px-8"
                  >
                    {verifyBusy ? <Loader2 size={16} className="animate-spin" /> : <ShieldCheck size={16} />}
                    Run the check
                  </button>
                  {verifyMessage && <p className="mt-3 rounded-lg bg-cream p-3 font-body text-sm text-ink-600">{verifyMessage}</p>}
                </div>
              </form>
            ) : (
              <div className="mt-5">
                  ...
                  <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4">
                    ...
                    <div>
                      ...
                      <p className="mt-1 font-body text-xs text-amber-700">
                        Both records were retrieved, but we couldn't find a recognisable date of birth field on one
                        or both. Please pick the modification type yourself below.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                    ...
                    <div>
                      ...
                      <p className="mt-1 font-body text-xs text-emerald-700">
                        BVN: {matchResult.bvn_date_of_birth} · NIN: {matchResult.nin_date_of_birth}. Your date of
                        birth is not the issue — pick whichever other field needs correcting below.
                      </p>
                    </div>
                  </div>
                ) : (
                  ...
                  return (
                    <label key={field.key} className="block font-body text-sm font-medium text-ink-600">
                      ...
                      {field.input === 'select' ? (
                        ...
                        <div className="mt-1">
                          {values[field.key] ? (
                            <div className="flex items-center gap-3 rounded-xl border border-parchment-line bg-cream p-2">
                              <img src={values[field.key]} alt={field.label} className="h-16 w-16 rounded-lg object-cover" />
                              <div className="flex-1">
                                <p className="font-body text-xs text-ink-600">Photo selected</p>
                                <label className="cursor-pointer font-body text-xs font-semibold text-gold-700 underline">
                                  Change photo
                                  <input
                                    type="file"
                            ...
                            <label className="flex cursor-pointer items-center justify-center rounded-xl border border-dashed border-parchment-line bg-cream p-4 text-center">
                              <span className="font-body text-xs text-ink-400">Tap to take a photo or upload one from your device</span>
                              <input
                                required={field.required}
                                type="file"
                                accept="image/png,image/jpeg,image/webp"
      ...
      <BvnModificationConsent open={consent} onAgree={() => setConsent(false)} onClose={() => nav('/dashboard')} />
    </AppShell>
  );
}
