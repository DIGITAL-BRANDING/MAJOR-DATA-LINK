export function BvnModificationConsent({ open, onAgree, onClose }: { open: boolean; onAgree: () => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-ink/70 p-4">
      <section className="flex max-h-[88vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-cream shadow-2xl">
        <header className="bg-ink px-6 py-5 text-center font-display text-xl font-bold text-gold-300">
          Consent &amp; Authorization Agreement
        </header>
        <div className="overflow-y-auto p-6 font-body text-sm leading-6 text-ink-600">
          ...
          <p className="mt-4 font-semibold text-ink">I agree to the terms above and authorize this platform to proceed with my BVN modification.</p>
        </div>
        <footer className="flex justify-center gap-3 border-t border-parchment-line p-4">
          <button onClick={onClose} className="rounded-xl bg-rose-600 px-5 py-2.5 font-body font-semibold text-white">
            Not Agreed
          </button>
          <button onClick={onAgree} className="rounded-xl bg-emerald-600 px-5 py-2.5 font-body font-semibold text-white">
            I Agreed
          </button>
        </footer>
      </section>
