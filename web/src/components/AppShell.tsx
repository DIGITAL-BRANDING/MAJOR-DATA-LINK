...
import { Bell, ChevronDown, Fingerprint, GraduationCap, IdCard, LayoutDashboard, LogOut, Menu, MessageCircle, PackageOpen, ReceiptText, Smartphone, WalletCards, Wifi, X, Users, Briefcase } from 'lucide-react';
...
const groups = [
  { label: 'VTU Services', icon: Wifi, image: '/branding/logo.png', items: [['Buy Data', '/buy-data'], ['Buy Airtime', '/buy-airtime']] },
  { label: 'NIN Services', icon: IdCard, image: '/branding/Validation.png', items: [['All NIN Services', '/nin-services']] },
  { label: 'BVN Services', icon: Fingerprint, image: '/branding/BVN Verifications.png', items: [['All BVN Services', '/bvn-services']] },
  { label: 'Result Checkers', icon: ReceiptText, image: '/branding/standard slip.jpg', items: [['WAEC / NECO / NABTEB', '/result-checkers']] },
  { label: 'JAMB Services', icon: GraduationCap, image: '/branding/premium slip.jpg', items: [['JAMB documents & admission', '/jamb-services']] },
  { label: 'CAC Services', icon: Briefcase, image: '/branding/CAC Services.png', items: [['CAC registration & verification', '/cac-registration']] },
  { label: 'My Deliveries', icon: PackageOpen, image: '/branding/information slip.jpg', items: [['Download my files', '/deliveries']] },
] as const;

function ServiceImage({ src, label, Icon }: { src: string; label: string; Icon: typeof Wifi }) {
  return <span className="relative flex h-7 w-7 shrink-0 overflow-hidden rounded-lg border border-white/30 bg-white/10"><img src={src} alt="" className="h-full w-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; }} /><Icon className="absolute inset-0 m-auto text-current" size={16} aria-label={label} /></span>;
}

function Sidebar({ close }: { close?: () => void }) {
  const { logout } = useAuth(); const nav = useNavigate(); const [open, setOpen] = useState('VTU Services');
  const go = (to: string) => { close?.(); nav(to); };
  return <aside className="premium-sidebar flex h-full w-72 flex-col px-3 py-5"><Link to="/dashboard" onClick={close} className="mb-8 px-3"><Logo /></Link><nav className="flex-1 space-y-1"><NavLink to="/dashboard" onClick={close} className={({isActive}) => `premium-nav-link flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold ${isActive ? 'is-active' : ''}`}><LayoutDashboard size={18}/>Dashboard</NavLink><NavLink to="/fund-wallet" onClick={close} className={({isActive}) => `premium-nav-link flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold ${isActive ? 'is-active' : ''}`}><WalletCards size={18}/>Fund Wallet</NavLink><NavLink to="/referrals" onClick={close} className={({isActive}) => `premium-nav-link flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold ${isActive ? 'is-active' : ''}`}><Users size={18}/>Referrals</NavLink><NavLink to="/support" onClick={close} className={({isActive}) => `premium-nav-link flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold ${isActive ? 'is-active' : ''}`}><MessageCircle size={18}/>Support & requests</NavLink>{groups.map(({label, icon: Icon, image, items}) => <div key={label}><button onClick={() => setOpen(open === label ? '' : label)} className="premium-nav-link flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold"><ServiceImage src={image} label={label} Icon={Icon}/><span className="flex-1 text-left">{label}</span><ChevronDown size={16} className={open === label ? 'rotate-180' : ''}/></button>{open === label && <div className="premium-subnav ml-9 border-l pl-3">{items.map(([label, to]) => <button key={to} onClick={() => go(to)} className="block w-full rounded-lg px-3 py-2 text-left text-xs">{label}</button>)}</div>}</div>)}</nav><button onClick={() => { logout(); close?.(); nav('/login'); }} className="premium-logout flex items-center gap-3 px-3 py-4 text-sm font-semibold"><LogOut size={18}/>Logout</button></aside>;
}
