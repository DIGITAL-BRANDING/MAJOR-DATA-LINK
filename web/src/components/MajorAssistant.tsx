...
type Workflow = { id: string; title: string; titleHa: string; fields: WorkflowField[]; purchaseEndpoint?: string; submitEndpoint?: string; statusEndpoint?: string; async: boolean; priceMode: 'result' | 'verification' | 'none'; priceServiceKeyTemplate?: string; status: 'active' | 'guided' };
  ...
  async function genericReview(workflow: Workflow, current: Record<string, unknown>) {
    ...
    try {
      ...
      if (workflow.priceMode === 'result') {
        ...
        const key = workflow.priceServiceKeyTemplate.replace(/\{(\w+)\}/g, (_m, k: string) => String(current[k] ?? '').toUpperCase());
  ...
  async function confirmGenericPurchase(pin: string) {
    ...
    try {
      ...
      if (activeWorkflow.async && activeWorkflow.submitEndpoint) {
        ...
        add({ text: res.status ? `${tr(res.message ?? 'Your request was successful.', res.message ?? 'Buƙatarka ta yi nasara.')}${delivery}` : tr(`${res.message ?? 'Request failed.'} Your wallet has not been charged for a failed request.`, `${res.message ?? 'Ya gaza.'} Ba za a cire maka kuɗi ba idan ya gaza.`) });
        reset();
      }
    } catch (error) {
  ...
  async function pollGenericTicket(statusEndpointTemplate: string, ticketId: string) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (status === 'success') { add({ text: tr(`Your request (${ticketId}) is complete. Check Transactions for the full result.`, `Buƙatarka (${ticketId}) ta shirya. Duba Transactions don cikakken sakamako.`) }); reset(); return; }
