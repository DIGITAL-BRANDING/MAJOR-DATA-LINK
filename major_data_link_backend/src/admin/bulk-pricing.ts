...
export function registerBulkPricingRoutes(router: Router) {
  ...
  router.post('/bulk-pricing/default-markup', formParser, async (req, res) => {
    const admin = requireFinanceOrSuper(req);
    if (!admin) return res.redirect('/admin/login');

    const percent = parseNonNegativeNumber(req.body.dataPlanMarkupPercent);
    const naira = parseNonNegativeNumber(req.body.dataPlanMarkupNaira);
  ...
  router.post('/bulk-pricing/data-plans', formParser, async (req, res) => {
    const admin = requireFinanceOrSuper(req);
    if (!admin) return res.redirect('/admin/login');

    const percent = parseNonNegativeNumber(req.body.markupPercent);
    const naira = parseNonNegativeNumber(req.body.markupNaira);
    const network = typeof req.body.network === 'string' && req.body.network.trim() ? req.body.network.trim() : undefined;
    if (percent === null || naira === null) {
      return res.redirect('/admin/bulk-pricing?flash=' + encodeFlash('error', 'Markup % and ₦ must both be valid numbers ≥ 0.'));
    }
  ...
  router.post('/bulk-pricing/services', formParser, async (req, res) => {
    const admin = requireFinanceOrSuper(req);
    if (!admin) return res.redirect('/admin/login');

    const percent = parseNonNegativeNumber(req.body.markupPercent);
    const naira = parseNonNegativeNumber(req.body.markupNaira);
    const provider = req.body.provider === 'alrahuz' ? 'alrahuz' : req.body.provider === 'techhub' ? 'techhub' : null;
