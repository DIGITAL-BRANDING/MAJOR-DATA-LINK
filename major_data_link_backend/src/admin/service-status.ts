...
type Row = { service: string; label: string; provider: string; is_active: boolean };
...
export function registerServiceStatusRoutes(router: Router) {
  router.get('/service-status', async (req, res) => {
    ...
    try {
      const [pins, verification] = await Promise.all([listServicePricesForAdmin(), listVerificationPricesForAdmin()]);
      const rows: Row[] = [...pins.map((row) => ({ ...row, provider: 'alrahuz' })), ...verification];
