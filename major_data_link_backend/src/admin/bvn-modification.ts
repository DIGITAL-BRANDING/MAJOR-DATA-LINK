 ...
 * document an admin needs to open to process a BVN Modification request by
 * hand, so it streams the real PDF bytes back so it opens directly in the
 * browser. Every open is written to AdminAuditLog. Reached via the
 * "Download PDF" admin action's redirectUrl on the Transaction resource.
 */
