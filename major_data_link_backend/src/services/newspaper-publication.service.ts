...
/**
 * Newspaper Publication (name-change notice) — ported from infoverify
 * (DIGITAL-BRANDING/maria-digital-services). Same manual-review pattern as
 * bvn-crm.service.ts: no provider API (this is an actual print-newspaper
 * affidavit + publication placed by an admin with BluePrint or DailyTrust),
 * so submitNewspaperPublicationRequest() debits the wallet, generates a PDF
 * of exactly what the customer submitted, and leaves the transaction
 * PENDING for an admin to place the publication.
 *
 * One deliberate change from infoverify's version: the completed newspaper
 * cutting is delivered via MAJOR-DATA-LINK's own user-delivery.service.ts
 * ("My Deliveries" inbox, same as JAMB documents - see admin/jamb.ts) rather
 * than stashed inside the transaction's sealed PII. That's the established
 * MAJOR-DATA-LINK convention for "admin hands the customer a finished
 * document" and keeps this consistent with JAMB instead of introducing a
 * second, different delivery mechanism.
 *
...
export function renderNewspaperPublicationPdf(params: { reference: string; values: Record<string, unknown>; submittedAt: Date }): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(16).font('Helvetica-Bold').text('MAJOR DATA-LINK \u2014 Newspaper Publication Request', { align: 'center' });
    doc.moveDown(0.3);
    doc
      .fontSize(10)
      .font('Helvetica')
    ...
    doc.font('Helvetica-Bold').fontSize(11).fillColor('#6b4f0b').text('Old Details');
    ...
    doc.moveDown(0.6);
    doc.font('Helvetica-Bold').fontSize(11).fillColor('#6b4f0b').text('New Details');
    doc.fillColor('#000').moveDown(0.4);
 ...
 *  PDF) for the admin's manage page. Never call this from a user-facing
 *  endpoint. */
