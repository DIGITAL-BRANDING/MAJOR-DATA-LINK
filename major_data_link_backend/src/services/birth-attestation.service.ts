...
/**
 * Birth Attestation — ported from infoverify (DIGITAL-BRANDING/maria-digital-services).
 * Same manual-review pattern as bvn-crm.service.ts/newspaper-publication.service.ts:
 * there is no provider API for this (NPC has no developer platform Techhub
 * or FranceVerified exposes), so submitBirthAttestationRequest() debits the
 * wallet, generates a PDF of exactly what the customer submitted (including
 * the uploaded photo), and leaves the transaction PENDING for an admin to
 * process the actual attestation with NPC by hand.
 *
 * As with Newspaper Publication, the final NPC-issued document is delivered
 * via user-delivery.service.ts ("My Deliveries") rather than infoverify's
 * approach of stashing it in the transaction's sealed PII - see
 * admin/birth-attestation.ts.
 *
 * Only one variant, matching the reference design's own single dropdown
 * option: "NPC Birth Attestation & Instant approval".
 */
...
export type BirthAttestationField = {
  ...
  options?: string[];
  /** Groups fields on the frontend form (e.g. "Father's Details") - purely presentational. */
  section?: string;
};
...
export function renderBirthAttestationPdf(params: { reference: string; values: Record<string, unknown>; submittedAt: Date }): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(16).font('Helvetica-Bold').text('MAJOR DATA-LINK \u2014 Birth Attestation Request', { align: 'center' });
    doc.moveDown(0.3);
    ...
    for (const field of BIRTH_ATTESTATION_FIELDS) {
      ...
      if (field.section && field.section !== currentSection) {
        ...
        doc.moveDown(0.6);
        doc.font('Helvetica-Bold').fontSize(11).fillColor('#6b4f0b').text(currentSection);
        doc.fillColor('#000');
    ...
    if (imageField) {
      ...
      if (typeof raw === 'string' && raw.startsWith('data:image/')) {
        try {
          ...
          // A malformed photo shouldn't fail the whole submission PDF -
          // every text field still gets through.
          console.error('[birth-attestation-pdf] failed to embed photo, continuing without it:', error);
...
/** Decrypts the sealed PII (submitted values, including the photo, and the
 *  auto-generated submission PDF) for the admin's manage page. Never call
