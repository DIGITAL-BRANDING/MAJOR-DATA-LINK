...
/**
 * BVN Modification — ported from infoverify (DIGITAL-BRANDING/maria-digital-services).
 * Same manual pattern as nin-modification.service.ts, for BVN instead:
 * there is no provider API for updating a BVN's details, so
 * submitBvnModificationRequest() debits the wallet, generates a PDF of
 * exactly what the customer submitted, and leaves the transaction PENDING.
 * An admin then processes the change by hand (bank/NIBSS agent portal) and,
 * from the Transaction admin page, either marks it complete
 * (completeBvnModification()) or rejects it (the existing generic
 * "reverse" action, which refunds).
 *
    ...
    if (imageField) {
      ...
      if (typeof raw === 'string' && raw.startsWith('data:image/')) {
        try {
          ...
          console.error('[bvn-modification-pdf] failed to embed photo, continuing without it:', error);
...
// BVN slip and their NIN slip (each a REAL, separately-priced purchase -
// reusing purchaseBvnSlip/purchaseNinByNin from verification.service.ts
// exactly as the BVN Verification and NIN Verification pages do, so this
// costs whatever those already cost via whichever provider is currently
// configured for each - see verification.service.ts's provider dispatch -
// not a separate fee) and compare the date of birth on each.
//
// Neither Techhub nor FranceVerified documents a single fixed key name for
// "date of birth" across every endpoint response, so `extractDob` below
// tries a handful of the field-name variants actually seen from both
// providers rather than assuming one. If neither slip's data contains a
// recognisable DOB field at all, the result is reported as "couldn't
// compare automatically" rather than a silent false "match" - the customer
// is shown both raw records instead and asked to compare by eye.
 ...
 *  differences without trying to fully parse every possible date format a
