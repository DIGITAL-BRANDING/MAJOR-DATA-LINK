...
function zodFor(field: BvnModificationField): ZodTypeAny {
  ...
  switch (field.input) {
    ...
    case 'date':
      ...
      break;
    case 'image':
      // A data URL string - same convention as birth-attestation.routes.ts's
      // image field. Capped well under express.json()'s 8mb body limit (see
      // app.ts) to leave room for the rest of the request.
      schema = z
        .string()
        .trim()
  ...
  // object-wide, in schemaFor()'s `.superRefine()` below.
  return field.required && !field.dependsOn ? schema : schema.optional().or(z.literal(''));
}

/** Builds the zod object straight from the type's field config, so a new field only ever needs updating in bvn-modification.service.ts. */
...
// real money (two real slip purchases, same price as BVN Verification + NIN
// Verification individually - nothing extra).
...
// One route per type, each validated against that type's own field config.
